/**
 * Client-side Anthropic API integration
 * Calls Anthropic API directly from the browser for true client-side privacy
 */

import Anthropic from '@anthropic-ai/sdk'
import { getTradesByBlock } from '@/lib/db/trades-store'
import { ALL_TRADE_TOOLS } from './trade-tools'
...
/**
 * Execute a tool call by fetching data from IndexedDB
 */
  ...
  try {
    ...
    switch (toolName) {
      case 'get_trades_by_block': {
        const { block_id, limit = 100, offset = 0 } = toolInput as {
          block_id: string
          limit?: number
          offset?: number
        }
        const allTrades = await getTradesByBlock(block_id)
        const limitedTrades = allTrades.slice(
          Number(offset),
          Number(offset) + Math.min(Number(limit), MAX_TRADES_LIMIT)
        )
      ...
      case 'get_trades_by_strategy': {
        const { block_id, strategy_name, limit = 100 } = toolInput as {
          block_id: string
        ...
        const limitedTrades = trades.slice(
          0,
          Math.min(Number(limit), MAX_TRADES_LIMIT)
        )
      ...
      case 'get_strategy_breakdown': {
        const { block_id, samples_per_strategy = 5 } = toolInput as {
          block_id: string
        ...
        const breakdown = Array.from(strategiesMap.entries()).map(
          ([strategyName, trades]) => {
            ...
            const avgLoss =
              losses.length > 0
                ? losses.reduce((sum, t) => sum + t.pl, 0) / losses.length
                : 0
...
/**
 * Call Anthropic API directly from the browser with tool support
 */
export async function callAnthropicWithTools(
  ...
  try {
    ...
    const lastUserMessage =
      anthropicMessages[anthropicMessages.length - 1]?.content.toLowerCase() || ''
    ...
    while (currentResponse.stop_reason === 'tool_use') {
      ...
      // Execute all tool calls client-side
